...
img_list = rmfield(meshData.list, 'type');
...
for i = length(meshData.list)+1:length(imgs)
    % name = sprintf('protein%i', i-length(meshData.list));
    name = compartmentlistCSGwithoutEC{i-length(meshData.list)};
    img = imgs{i};
    if ~any(img(:))
        continue;
    end
    ordinal = length(img_list)+1;
    img_list(i) = struct('name', name, 'ordinal', ordinal, 'img', img);
end
...
if any(ec_img(:))
    ...
    img_list(end+1) = struct('name', 'EC', 'ordinal', 1, 'img', ec_img);
end
...
for i = 1:length(img_list)
    ...
    mask = img > 0;
    if strcmpi(name, 'cell')
        % Treat cell like background
        mask = mask & all_objects_image == 0;
    end
    % all_objects_image(mask) = i;
...
% Validator different from specification:
SampledFieldGeometryNode.setAttribute([s,'isActive'],'true');
