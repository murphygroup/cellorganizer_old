...
% 01/23/2018 xruan change to not save the final results
% 03/11/2018 clean code and use it as part of spherica_parameterization_xr
% change name from initParamCALD.m to initParamCALD_x.m to avoid wrong
% call.
% 03/12/2018 change the method to choose poles, that is, we use the farest
% two points in the surface mesh, assuming the lengths of edges are the
% same. 

function [sph_verts, poles, dateline] = initParamCALD_x1(vertices, faces)
...
fn = size(faces,1);
if fn>maxfn
    fprintf('Reduce face number from %d to %d\n',fn,maxfn);
...
fprintf('%+4d = %d (%d) vertices * 2 - %d faces; %d edges\n', ...
...
[poles, direct2nd] = determine_poles_1(vertices, qfaces); % north 1 south 2
...
pca_dim = 3;
% xruan change to directly use matlab pca
...
fprintf('after pca min_max_number: %d %d\n',n,m);
...
function [poles, direc] = determine_poles_1(vertices, faces, bim)
% the idea is to find the diameter of the graph of the mesh. 
% 1. Construc ta graph based on the structure of the mesh
% 2. Every time pick N points, and find the largest shortest with all other points. 
% and pick the largest one. 
% 3. iteratively do that for all points, and finally get the pair as the
% diameter. 

% find edges and build graph
edges = [faces(:, 1:2); faces(:, 2:3); faces(:, 3:4); faces(:, [1, 4])];
edges = unique(sort(edges, 2), 'rows');
edges_length = sqrt(sum((vertices(edges(:, 1), :) - vertices(edges(:, 2), :)) .^ 2, 2));
G = graph(edges(:, 1), edges(:, 2), edges_length, numel(unique(edges(:))));

nvert = size(vertices, 1);
MaxSize = 1e8;
N = floor(MaxSize / nvert);
NIter = ceil(nvert / N);
...
for i = 1 : NIter
    query = (i - 1) * N + 1 : min(nvert, i * N);
    dist_mat = distances(G, query);
    [max_dist, max_ind] = max(dist_mat(:));
    [s_i, t_i] = ind2sub(size(dist_mat), max_ind);
    
    if max_dist > largest_dist
        largest_dist = max_dist;
        diameter_pair = [query(s_i), t_i];
    end
end

s_ind = diameter_pair(1);
t_ind = diameter_pair(2);
...
if vertices(s_ind,3) > vertices(t_ind,3)
    poles = [s_ind t_ind];
else
    poles = [t_ind s_ind];
end
...
max_j = find(pc2v==max(pc2v(IDX)))'; max_j = max_j(1);
% 04/06/2018 xruan fix bug if max_j is the same as a pole. 
if any(poles == max_j)
    [~, sorted_inds] = sort(abs(pc2v), 'descend');
    for i = 1 : numel(sorted_inds)
        max_j = sorted_inds(i);
        if ~any(poles == max_j)
            break;
        end            
...
while (here~=landmarks(2))
    ...
    if length(dateline) > 1
        ...
        if (length(next) > 1)
            nb_verts2 = nb_verts(next,:);
            % 02/04/2018 xruan fix bug for distance computing
            % concord2 = abs(sum(nb_verts2.*repmat(normal2surf,[length(next),1])));
            concord2 = abs(nb_verts2 * normal2surf');
...
for i=2:length(dateline)-1
    ...
    while ~isempty(related_faces)
        ...
        switch length(ii)
            ...
            otherwise
                fprintf('There are %d choices. Investigate!\n',length(ii)); break;
        end
    end
    pts = pts';
...
theta = full(theta);
fprintf('North Pole : %05d (%f, %f) pi\n',landmarks(1),0,-0.5);
fprintf('South Pole : %05d (%f, %f) pi\n',landmarks(2),0,0.5);

values = phi.^2 + (theta+pi/2).^2;
[value, east] = min(values);
landmarks(3) = east; % center of east hemisphere
fprintf('East Center: %05d (%f, %f) pi\n',east,theta(east)/pi,phi(east)/pi);

values = phi.^2 + (theta-pi/2).^2;
[value, west] = min(values);
landmarks(4) = west; % center of west hemisphere
fprintf('West Center: %05d (%f, %f) pi\n',west,theta(west)/pi,phi(west)/pi);
...
if ~isempty(ix)
    fprintf('%d longitudes == 0, do something\n',length(ix)); return;
end
