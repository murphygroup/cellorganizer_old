...
if ~isempty(cell_spharm_descriptor)
    ...
    Zvert_cell = real(Zs * fvec_cell .* synthesis_scale_factor);
...
if ~isempty(nuc_spharm_descriptor)
    ...
    Zvert_nuc = real(Zs * fvec_nuc .* synthesis_scale_factor);
...
result = struct('nucimg', nucimg, 'cellimg', cellimg, 'nucmesh', nuc_mesh, 'cellmesh', cell_mesh);
