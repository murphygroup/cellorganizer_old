...
train_score = model.cellShapeModel.train_score;
...
all_spharm_descriptors = model.cellShapeModel.all_spharm_descriptors;
if numel(model.cellShapeModel.components) > 1
    ...
    mesh_func = @(x) reconstruct_spharm_descriptor_to_mesh(all_spharm_descriptors(:, :, :, x), model.cellShapeModel.components);
else
    mesh_func = @(x) reconstruct_spharm_descriptor_to_mesh(all_spharm_descriptors(:, :, x), model.cellShapeModel.components);
end    
...
%     try
        ...
        colormap_func = get_colormap(labels(cellind));
        param.colormap_func = colormap_func;
