function [r_structure] = Greedy3D_lambda_pre(source, target, lambda_list, options)
  ...
  % 2016-03-30 xruan: change source_compress, target_compress to source,
  % target to make the synthesize work; change some functions for synthesis
  % to faster computing framework. 
  %tic
  start_wall_time = tic;
...
%   clear source target
  ...
  r = Greedy3D_integration(source, target, options);
    ...
    if ~options.just_compute_distance
      ...
      elseif isinf(lambda)
        ...
        if options.save_intermediates || (options.known_distance >= 0 && options.stop_source_distance >= 0)
          ...
          r2 = Greedy3D_integration(source, target, options2);
          if (from_left_side)
        ...
        img = image_interp3(start_image, final_deformation, '*linear', 0);
      end
    end
